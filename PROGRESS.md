...
- **Results**: small real-data smoke (`reports/inject_spike/20251223_225141/`) shows baseline detection/acceptance = 0.0 and μ=3/6/9 detection/acceptance = 0.0 (acceptance drought; skip histogram empty), indicating no evaluated effect in this slice.
- **Tests**: `make test-fast` (73 passed, 168 deselected, 1 warning: PytestConfigWarning unknown timeout option).
- **Artifacts**: run log `docs/agent_runs/20251223_222840_ticket-18_inject-spike-sensitivity/`; outputs `reports/inject_spike/20251223_225141/` (curve.csv/curve.png/run.json/resolved_config.json/selected_windows.csv); partial runs `reports/inject_spike/20251223_224624/` (failed) and `reports/inject_spike/20251223_224638/` (interrupted).
