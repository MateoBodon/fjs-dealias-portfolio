...
- **Branch/Run**: `codex/ticket-18-inject-spike-sensitivity` (RUN_NAME=`20251223_222840_ticket-18_inject-spike-sensitivity`), git sha `88871be2abf3c54153b238eeed1932ba0c0c3c56`.
...
- **Results**: larger slice run (`reports/inject_spike/20251224_051700/`, 80 windows) shows baseline detection/acceptance = 0.0 and μ=3/6/9/12/15 detection/acceptance = 0.0; `curve.csv` and `run.json` confirm `n_detected=0`, so failures are pre-gate (no candidate spikes). Earlier small run (`reports/inject_spike/20251223_225141/`) also shows zero detection/acceptance.
- **Tests**: `make test-fast` (73 passed, 168 deselected, 1 warning: PytestConfigWarning unknown timeout option).
- **Artifacts**: run log `docs/agent_runs/20251223_222840_ticket-18_inject-spike-sensitivity/`; outputs `reports/inject_spike/20251224_051700/` (curve.csv/curve.png/run.json/resolved_config.json/selected_windows.csv) and `reports/inject_spike/20251223_225141/`; partial runs `reports/inject_spike/20251223_224624/` (failed), `reports/inject_spike/20251223_224638/` (interrupted), and `reports/inject_spike/20251224_051229/` (interrupted).
