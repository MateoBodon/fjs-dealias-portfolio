...
source_event: Ticket 37 deterministic reference harness milestone 2
...
| The current production detector does not satisfy the reference contract. | Supported as current blocker | `tools/check_fjs_reference.py`; `make detector-reference-gate`; three strict expected failures in `tests/fjs/test_reference_oracle.py`. | Five fail-loud issues block null/power calibration and broad experiments; do not treat expected failures as waived defects. | 2026-07-10 deterministic gate |
