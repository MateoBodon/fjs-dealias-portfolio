...
source_event: Ticket 37 production reference repair milestone 3
...
| Ticket 24 is a flat-zero exact-config negative control, not target-between-component power evidence. | Supported by source/command provenance | `docs/artifacts/detector-contract-reference/ticket24_week_full_fix/{manifest.json,curve.csv,gating_reasons.csv}`; historical source commit `31c05a57`; `docs/agent_runs/20251226_060917_ticket-24_finish-week-inject-spike/COMMANDS.md`; `tests/fjs/test_detector_contract.py`. | The run used an iid observation-level injection and carries no component-mode field. Target-power assessment now requires `inject_mode=between`. | 2026-07-10 source, command, hash, and reducer audit |
| The independent balanced one-way reference is deterministic and production-independent. | Supported for mechanism correctness | `src/fjs/reference_oracle.py`; `tests/fjs/test_reference_oracle.py`; `docs/strategy/FJS_REFERENCE_HARNESS_FINDINGS.md`. | Exact scalar and two-stratum values, invariances, deterministic oracle/sham controls, and spectral reconstruction are tested. This is not empirical performance evidence. | 2026-07-10 focused tests |
| Production satisfies the frozen deterministic reference contract. | Supported for bounded mechanism correctness | `src/fjs/{mp,dealias,reconstruction,overlay}.py`; `tests/fjs/test_reference_oracle.py`; `tests/fjs/test_reconstruction.py`; `make detector-reference-gate`. | Covers exact low-dimensional edge/root/t/design and reconstruction behavior only; it does not validate realistic detector size or performance. | 2026-07-10 Ticket 37 milestone 3 |
| The predeclared two-cell between fixture passes and is byte-reproducible. | Supported for synthetic mechanism plumbing | `docs/artifacts/detector-contract-reference/between_mechanism_v1/{input_spec.json,manifest.json,curve.csv,trials.csv}`; `tools/generate_fjs_between_fixture.py --check`. | Null is 0/12 and `mu=6` is 12/12 for detection/acceptance. Twelve trials and two cells are not a full power/size study or empirical evidence. | 2026-07-10 frozen replay at `9afbb72` |
...
| The ambitious flagship design is frozen before broad execution. | Supported as predeclaration | `docs/strategy/FJS_SCIENTIFIC_RECENTER_PREDECLARATION.md`. | The deterministic entry gate passes, but the full null/power/invariance/real-design suite remains open; no real-data performance result has been produced by Ticket 37. | 2026-07-10 |
