...
- **Week inject-spike full run (ticket-24)** — Completed full week curve on small daily panel (assets_top=20, window=30, horizon=5, μ=3/6/12/24). Detection/acceptance remain zero for all μ; pre-gate failures dominated by `tvec_off_component`, `tvec_no_real_root`, and `tvec_no_admissible_root` after reclassifying t-vector errors (run `reports/inject_spike/20251226_ticket24_week_full_fix/`, artifacts in `docs/agent_runs/20251226_060917_ticket-24_finish-week-inject-spike/`).
