...
- `runlog_init.py` → creates `docs/agent_runs/<RUN_NAME>/` with standard log files (including `META.md`)
- `validate_runlog.py` → validates required run log files (single run or `--all`)
